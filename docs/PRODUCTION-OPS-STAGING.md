...
- Export includes every matching row across all pages, as `.xlsx`. Columns: date, brand, platform, location, studio, start/end hour, host, optional Host ID, status, Session ID. Dates and hours are numeric Excel cells; identifiers remain text. Exporting does not expose fee/rate columns. Status/Session ID are informational, not import updates.
- Download the updated template without Quotation. The web review loads quotations for the file's date range (maximum 367 days), independently of the currently displayed schedule filter. Brand/platform/date identify candidates. A unique candidate is selected automatically; multiple candidates require selection by period. Missing quotations and aggregate quota overruns block submission. The existing schedule-import RPC revalidates all rows atomically. No additional migration is required beyond 007.
- Ringkasan Jadwal defaults to the current calendar month and supports daily/weekly/custom ranges plus brand/platform/location filters. Totals group all matching draft/published sessions by brand, platform and location; cancelled sessions are excluded. Mirror duration counts once. These are scheduled hours, not actual logbook hours.
- Planning/import still requires whole hours: `9` and `09:00` work; `9.30`, `09:30`, and fractional Excel time cells are rejected without rounding.
