...
const headers=['Tanggal','Quotation','Brand','Platform','Lokasi','Studio','Jam Mulai','Jam Selesai','Host','Host ID'];
sheet.getRange('A1:J1').values=[headers];
sheet.getRange('A1:J21').format.font={name:'Arial',size:10,color:'#252035'};
sheet.getRange('A1:J1').format={fill:'#7439C6',font:{name:'Arial',size:10,bold:true,color:'#FFFFFF'},rowHeight:28};
sheet.getRange('A2:J21').format.fill='#FFF9E8';
sheet.getRange('A1:J21').format.columnWidth=21;
sheet.getRange('B1:C21').format.columnWidth=26;
sheet.getRange('F1:F21').format.columnWidth=28;
sheet.getRange('I1:J21').format.columnWidth=32;
sheet.getRange('A2:A501').setNumberFormat('yyyy-mm-dd');
sheet.getRange('B2:F501').setNumberFormat('@');
sheet.getRange('G2:H501').setNumberFormat('0');
sheet.getRange('I2:J501').setNumberFormat('@');
sheet.getRange('D2:D501').dataValidation={rule:{type:'list',values:['TikTok','Shopee','Mirror']}};
sheet.getRange('E2:E501').dataValidation={rule:{type:'list',values:['jakarta','bandung']}};
sheet.dataValidations.add({range:'G2:G501',rule:{type:'whole',operator:'between',formula1:0,formula2:23}});
sheet.dataValidations.add({range:'H2:H501',rule:{type:'whole',operator:'between',formula1:1,formula2:24}});
...
guide.getRange('A3:B15').values=[
 ...
 ['Quotation','Nomor quotation yang sudah dibuat. Isi sebagai teks, termasuk nol di depan.'],
 ...
 ['Batas file','Maksimal 500 sesi dan 5 MB per file .xlsx. Format .xls lama perlu Save As .xlsx.'],
...
book.recalculate();
console.log((await book.inspect({kind:'table',range:'Jadwal!A1:J2',include:'values',tableMaxRows:2,tableMaxCols:10})).ndjson);
await fs.mkdir(output,{recursive:true});
for(const [name,range] of [['Jadwal','A1:J6'],['Panduan','A1:B15']]){
