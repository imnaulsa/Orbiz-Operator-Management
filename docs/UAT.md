...
10. Dua manager bersamaan melakukan publish/copy/izin untuk lokasi sama. Periksa tidak ada duplikasi, assignment terlarang, snapshot parsial atau deadlock. RPC manager memakai row lock per lokasi agar mutasi lokasi yang sama berjalan serial; pertahankan smoke test hosted ini pada regresi berikutnya.
...
12. Cost: cek distinct working days, total hours, custom range, bulan kalender, combined hanya Super Admin. Preset Mitra memakai tanggal 21 bulan sebelumnya sampai 20 bulan berjalan, inklusif; uji boundary tanggal 20 dan 21.
...
Release gate: seluruh 80 skenario desktop dan 34 skenario mobile tercatat Pass; CI typecheck, 29 unit/server tests, 91 database/RLS checks, production build, secret scan dan dependency audit harus tetap hijau. Preview: `https://deploy-preview-1--dynamic-unicorn-4cd699.netlify.app`. Merge `main` tetap membutuhkan approval eksplisit user setelah commit terakhir selesai diretest.
