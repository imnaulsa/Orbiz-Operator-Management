...
import {hostDailySummary,parseBestHourSlots,parseHours,type ProductionData} from '../src/lib/production';
describe('production domain',()=>{
 ...
 it('parses multi-hour best-hour blocks',()=>{expect(parseBestHourSlots('09-11, 19:00-22:00')).toEqual([{start:9,end:11},{start:19,end:22}]);expect(()=>parseBestHourSlots('11-09')).toThrow();expect(()=>parseBestHourSlots('24-25')).toThrow();});
