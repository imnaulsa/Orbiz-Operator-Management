...
export function QuotationPage(c:Ctx){
 ...
 return <>
  ...
  <section className="card"><h2>Quota tracker</h2><div className="table-wrap"><table>
   <thead><tr><th>Quotation / Brand</th><th>Shop ID</th><th>Platform</th><th>Periode</th><th>Kuota</th><th>Terplot</th><th>Sisa</th><th>Nilai quotation</th><th>Aksi</th></tr></thead>
   <tbody>{c.data.quotations.map(q=><tr key={q.id}>
    <td>{q.reference}<br/>{q.brand}</td><td>{q.account}</td><td>{q.platform}</td><td>{q.period_start} – {q.period_end}</td><td>{q.hours} jam</td><td>{q.allocated_hours} jam</td><td>{q.hours-q.allocated_hours} jam</td><td>{money(Math.round((q.rate??0)*q.hours/1000)*1000)}</td>
    <td><div className="row-actions compact"><button disabled={c.busy} onClick={()=>setEdit(q)}>Edit</button><button className="danger" disabled={c.busy} onClick={()=>{
...
export function MasterDataPage(c:Ctx){
 ...
 return <>
  <section className="card"><h2>Master Brand & Shop ID</h2>
   <form key={brand?.id??'new'} onSubmit={e=>void saveBrand(e)}><fieldset disabled={c.busy}><div className="form-grid">
   ...
   <p className="muted">Brand yang masih dipakai quotation tidak bisa dihapus atau diganti Shop ID-nya. Perubahan nama ikut memperbarui nama di quotation.</p>
   <div className="table-wrap"><table><thead><tr><th>Brand</th><th>TikTok</th><th>Shopee</th><th>Mirror</th><th>Aksi</th></tr></thead><tbody>{c.data.brands.map(b=><tr key={b.id}>
    <td>{b.name}</td><td>{b.shop_id_tiktok??'—'}</td><td>{b.shop_id_shopee??'—'}</td><td>{b.shop_id_mirror??'—'}</td><td><div className="row-actions compact">
  ...
  <section className="card"><h2>Master Studio & Lokasi</h2>
   <form key={studio?.id??'new'} onSubmit={e=>void saveStudio(e)}><fieldset disabled={c.busy}><div className="form-grid">
   ...
   <p className="muted">Studio yang masih memiliki jadwal tidak bisa dihapus atau dipindahkan lokasinya. Kapasitas tidak boleh kurang dari slot jadwal terpakai.</p>
   <div className="table-wrap"><table><thead><tr><th>Lokasi</th><th>Studio</th><th>Kapasitas</th><th>Aksi</th></tr></thead><tbody>{c.data.studios.map(s=><tr key={s.id}>
    <td>{s.location_id}</td><td>{s.name}</td><td>{s.capacity} live bersamaan</td><td><div className="row-actions compact">
