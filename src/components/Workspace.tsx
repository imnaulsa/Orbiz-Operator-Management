...
import {today,monday,addDays,prettyDate,calendarMonth,logbookPeriod,type Profile} from '../lib/domain';
...
const links=[['/production','Livestreaming Production ↗'],['/overview','Ringkasan'],['/plotting','Plotting Jadwal'],['/availability','Availability & Izin'],['/accounts','Operator Staff'],['/cost','Cost Operator']];
export function Workspace(){
 ...
 return <div className="app"><aside><div className="brand"><span className="logo">O</span><div><strong>Orbiz Ops</strong><small>Operator Management</small></div></div><p className="eyebrow">WORKSPACE</p><nav>{(staff?[['/production','Jadwal Live & Check ↗'],['/my-schedule','Jadwal & Logbook'],['/availability','Availability & Izin']]:links).map(([to,label])=><NavLink key={to} to={to}>{label}</NavLink>)}</nav><div className="identity"><strong>{profile.display_name}</strong><small>{profile.role.replaceAll('_',' ')}</small><button disabled={busy} onClick={()=>void run('Keluar',async()=>{const{error}=await db().auth.signOut();if(error)throw error;})}>Keluar</button></div></aside><main className="workspace"><header><div><p className="eyebrow">ORBIZ / OPERATOR MANAGEMENT</p><h1>{title}</h1></div>{profile.role==='super_admin'?<div className="segmented">{['jakarta','bandung'].map(c=><button disabled={busy} className={c===city?'selected':''} key={c} onClick={()=>{setCity(c);setMessage('');setActionError('');}}>{c==='jakarta'?'Jakarta':'Bandung'}</button>)}</div>:<span className="pill">{location==='jakarta'?'Jakarta':'Bandung'}</span>}</header><div className="toolbar"><label>Dari<input type="date" value={start} max={end} disabled={busy} onChange={e=>{if(e.target.value)setStart(e.target.value);}}/></label><label>Sampai<input type="date" value={end} min={start} disabled={busy} onChange={e=>{if(e.target.value)setEnd(e.target.value);}}/></label><button disabled={busy} onClick={()=>resetDates(monday(today()),addDays(monday(today()),6))}>Minggu ini</button><button disabled={busy} onClick={()=>{const p=calendarMonth(today());resetDates(p.start,p.end);}}>Bulan ini</button>{path==='/my-schedule'&&<button disabled={busy} onClick={()=>{const p=logbookPeriod(start);resetDates(p.start,p.end);}}>Periode 21–20</button>}<small className="sync">WIB · {live?'Pembaruan langsung aktif':'Memeriksa pembaruan setiap 15 detik'}</small></div>{error&&data.profiles.length>0&&<div className="notice" role="status">Pembaruan data gagal. Menampilkan data terakhir; tunggu pembaruan berhasil sebelum mengubah jadwal. <button onClick={()=>setRevision(r=>r+1)}>Coba lagi</button></div>}{message&&<div className="notice success" role="status">{message}</div>}{actionError&&<div className="notice error" role="alert">{actionError}</div>}<div key={location+':'+path} aria-busy={busy}>{content}</div><footer>Orbiz Operator Management · Asia/Jakarta</footer></main></div>;
}
