...
export default async (request: Request) => {
  ...
  if(process.env.NETLIFY==='true'&&(process.env.CONTEXT==='production'||!process.env.STAGING_SUPABASE_PROJECT_REF||url!==`https://${process.env.STAGING_SUPABASE_PROJECT_REF}.supabase.co`))return response(503,'Production Ops preview wajib memakai database staging');
  ...
  if (!actor?.active || !['super_admin','operator_manager','host_manager'].includes(actor.role)) return response(403, 'Akses ditolak');
  ...
  if (typeof email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || typeof name !== 'string' || !name.trim() || name.length > 120 || !['staff', 'operator_manager', 'super_admin','host_manager','host','admin_sales'].includes(String(role)) || !['jakarta','bandung'].includes(String(location)) || !['internal','mitra'].includes(String(employment))) return response(400, 'Identitas akun tidak valid');
  if (['staff','host'].includes(String(role)) && (typeof fee !== 'number' || !Number.isFinite(fee) || fee < 0 || fee > 999999999999 || typeof effective !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(effective) || Number.isNaN(Date.parse(effective)))) return response(400, 'Fee dan effective date wajib');
  if (actor.role !== 'super_admin' && (role !== (actor.role==='host_manager'?'host':'staff') || location !== actor.location_id)) return response(403, 'Hanya anggota tim lokasi sendiri');
  ...
  const { error: profileError } = await client.rpc('manage_account', { p_id: data.user.id, p_name: name.trim(), p_role: role as Database['public']['Enums']['app_role'], p_location: String(location), p_employment: employment as 'internal' | 'mitra', p_active: true, ...(['staff','host'].includes(String(role)) ? { p_initial_fee: fee as number, p_effective: effective as string } : {}) });
