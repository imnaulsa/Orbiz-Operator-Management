...
const request=(change={},token=true,origin='https://test.example')=>new Request(`${origin}/.netlify/functions/accounts`,{method:'POST',headers:{Origin:origin,'Content-Type':'application/json',...(token?{Authorization:'Bearer test-token'}:{})},body:JSON.stringify({...body,...change})});
describe('Netlify account boundary',()=>{
 it('accepts a deploy subdomain belonging to the configured Netlify site',async()=>{process.env.URL='https://orbiz.netlify.app';expect((await handler(request({},true,'https://deploy-preview-1--orbiz.netlify.app'))).status).toBe(201);});
 it('rejects a deploy subdomain belonging to another Netlify site',async()=>{process.env.URL='https://orbiz.netlify.app';expect((await handler(request({},true,'https://deploy-preview-1--attacker.netlify.app'))).status).toBe(403);expect(mock.invite).not.toHaveBeenCalled();});
